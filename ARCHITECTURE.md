...
- The drag overlay is portaled to `document.body` so the page-entry transform cannot offset fixed-position coordinates. It reuses the complete issue card, places its top edge directly below the pointer through `client/src/pages/issueBoardDrag.ts`, shows a grabbing cursor, leaves a translucent source placeholder, and strengthens the active-column highlight.
