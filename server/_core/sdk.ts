...
class SDKServer {
  ...
  normalizeEmail(email: string): string {
    ...
    if (!isValid) {
      throw ForbiddenError("Invalid email address");
    }
    return normalized;
  }

  private buildEmailOpenId(email: string): string {
    const digest = createHash("sha256").update(email).digest("hex").slice(0, 32);
    return `${EMAIL_OPEN_ID_PREFIX}${digest}`;
  }
  ...
  async loginWithEmail(email: string): Promise<AuthenticatedUser> {
    ...
    const openId = this.buildEmailOpenId(normalizedEmail);
    await db.upsertUser({
      openId,
  ...
  async authenticateRequest(req: Request): Promise<AuthenticatedUser> {
    ...
    const session = sessionToken ? await this.verifySession(sessionToken) : null;
